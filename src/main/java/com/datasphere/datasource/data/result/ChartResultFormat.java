...
import static com.datasphere.datasource.data.result.SearchResultFormat.ResultType.MATRIX;
    ...
    if (CollectionUtils.isNotEmpty(aggrs)) {
      for (Field field : aggrs) {
        if (field instanceof MeasureField ||
            (field instanceof ExpressionField && ((ExpressionField) field).getRole() == com.datasphere.datasource.Field.FieldRole.MEASURE)) {
          aggregation.add(
              new PivotResultFormat.Aggregation(field.getAlias())
          );
        }
